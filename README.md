...
On other platforms configure with `cmake -S . -B build -DBUILD_TESTING=ON`, build with `cmake --build build`, then run `ctest --test-dir build --output-on-failure`. The optional preset selects MinGW and is Windows-specific. Each track can also be configured independently.

All results stay inside their owning module's `results/` directory. ML result subfolders match the C++ source filename, such as `04_end_to_end_results` and `predict_results`. The Titanic capstone uses one shared fitted pipeline for feature definitions, training-only imputation/scaling/PCA, evaluation and model reload. K-means, hierarchical clustering and silhouette evaluation are explicit unsupervised priorities.
