...
## Purpose
This folder gets your machine ready and runs your first program.
...
|------|--------|-----------------|----------------|
| `hello.cpp` | `hello` | Your first C++ program | prints only |
...
|------------------|--------------|
| `#include <iostream>` | Loads the input/output stream library so you print text with `std::cout`. |
...
| `__cplusplus` | A magic number the compiler fills in: `201703` means C++17. |
...
Everything after this lesson is more `main()` functions that use
variables, loops, classes, and files. The skeleton you see here
never changes.
