...
module: `include/dsts/sampling.hpp` — `train_test_split()`,
...
| `04_boot_sample_ci.cpp` | `s08_04_boot` | Seeded indices; bootstrap CI of a correlation; data: tips.csv (real data, 244 restaurant tips) | `rng.choice(..., replace=True)` | prints only |
...
- **Ordering is a bias** — the first 40 tips rows contain zero Thursdays;
  any random 40 contain ~10.
- **Re-splitting shows the wobble** — the test mean tip swings ±0.25 across
  seeds at 60/40 but tightens at 80/20; that spread is what k-fold averages out.
