...
ones are summarised here so a new module can be written against them.
...
- The run name and every relative path must be non-empty, relative and free
  of `..`; anything else throws `std::invalid_argument`, so a lesson can never
  write outside its own results folder.
