...
**Method.** Turn raw messages into something a model can count. The tokenizer in
...
source line number of every row so any result can be traced back to the file.

**Core idea.** Every later text model sees only what the tokenizer produces.
Decisions made here - lowercasing, no stemming, no stop-word list, duplicates
removed *before* splitting - shape the vocabulary, the class balance and the
honesty of the train/test split. The end-to-end project therefore trains a
Multinomial Naive Bayes classifier on raw token counts to show the effect of
those choices on a real spam filter.
...
Output paths are compiled in (`RUN_OUTPUT_DIR`), so the executables can be
...
- Deduplicate before splitting or the same message can leak into train and test.
- The tokenizer is the first model: what it discards can never be recovered.
- Keep row ids so every prediction can be traced to a source line.
