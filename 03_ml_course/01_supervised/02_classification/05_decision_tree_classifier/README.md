...
`penguins` (342 complete cases, 3 species) in the lesson because two or three splits already separate the species and the tree can be printed. The end-to-end project uses `breast_cancer` with max_depth in {2, 4, 8}. Datasets are CSV files under `03_ml_course/helper/data/` and are loaded through `helper/data/datasets.hpp`; the build passes their folder as `DATA_DIR`.
...
| `predict.cpp` | `cdtc_predict` | `#include`s `04_end_to_end.cpp` so the same code serves inference, compiled with its own `RUN_OUTPUT_DIR`; wraps it in a `main` that refuses to run without `--predict` | `results/predict_results/predictions.csv` |
