...
`lda_numerical` runs `tests/model_test.cpp`. The fixture checks are listed in the table. Any failed check throws, so the exit code is non-zero. `lda_workflow` runs `lda_end_to_end --quick`. It fails if the pipeline or its reload verification fails.
