...
| `03_implementation.cpp` | `cmlp_implementation` | MLP versus logistic regression on wine | `results/03_implementation_results/`: `c10_mlp.split` |
...
| `predict.cpp` | `cmlp_predict` | `#include`s `04_end_to_end.cpp` so the same code serves inference, compiled with its own `RUN_OUTPUT_DIR`; wraps it in a `main` that refuses to run without `--predict` | `results/predict_results/predictions.csv` |
