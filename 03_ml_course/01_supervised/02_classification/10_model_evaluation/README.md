...
| `predict.cpp` | `eval_predict` | `#include`s `04_end_to_end.cpp` so the same code serves inference, compiled with its own `RUN_OUTPUT_DIR`. It wraps it in a `main` that refuses to run without `--predict` | `results/predict_results/predictions.csv` |
...
- Bootstrap intervals in `evaluation/metrics.json` tell you whether two models differ.
