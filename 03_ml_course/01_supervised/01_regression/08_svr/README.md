...
`svr_numerical` runs `tests/model_test.cpp` (the fixture checks listed in the table; any failed check throws, so the exit code is non-zero). `svr_workflow` runs `svr_end_to_end --quick` and fails if the pipeline or its reload verification fails.
