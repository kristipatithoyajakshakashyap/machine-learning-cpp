...
| `03_implementation.cpp` | `elastic_implementation` | Exact tiny problem plus a synthetic sparse problem, with a coefficient plot | `results/03_implementation_results/`: `coefficients.csv`, `coefficients.svg` |
...
`elastic_numerical` runs `tests/model_test.cpp`. The fixture checks are listed in the table. Any failed check throws, so the exit code is non-zero. `elastic_workflow` runs `elastic_end_to_end --quick`. It fails if the pipeline or its reload verification fails.
